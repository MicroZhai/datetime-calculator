function renderPart(p,ri,pi){
  ...
  function renderNormalized(){
    ...
    if(raw&&cp.length){
      const ms=partsMs(cp);
      if(ms===null){normalizedEl.innerHTML='<strong>当前输入超出安全范围</strong>';return}
  ...
  function render(){
    ...
    if(r.ok){
      ...
      resultEl.textContent='—';secondaryEl.textContent='超出安全范围';setError(r.error);
    }
  ...
  function clearAll(show=true){
    ...
    formatIndex=0;lastResultMs=0;justEvaluated=false;setError('');render();
    if(show)notify('已清空');
  }
  ...
  function commitPartEdit(){
    ...
    if(p.kind==='unit'){
      ...
      const n=Number(p.buffer);
      if(!Number.isFinite(n)){notify('数字格式不正确');return false}
      if(valueToMs(n,p.unit)===null){notify('该时间值过大');setError('时间数值过大');return false}
      rows[p.rowIndex].parts[p.partIndex]={kind:'unit',unit:p.unit,value:n};
    }else{
      ...
      const candidate={kind:'colon',hours:String(Number(p.hours)),minutes:String(Number(p.minutes)).padStart(2,'0'),
        seconds:p.hasSeconds?String(Number(p.seconds)).padStart(2,'0'):null};
      if(partMs(candidate)===null){notify('该时间值过大');setError('时间数值过大');return false}
      rows[p.rowIndex].parts[p.partIndex]=candidate;
    }
  ...
  function appendToNumberBuffer(d){
    ...
    if(numberBuffer.length>=18){notify('数字位数过长');return}
  ...
  function inputDigit(d){
    ...
    if(partEdit){
      ...
      for(const ch of d.split('')){
        if(f==='hour'){if(v.length>=15){notify('小时位数过长');break}v+=ch}
        else{
  ...
  function commitUnit(unit){
    ...
    const n=Number(numberBuffer);
    if(!Number.isFinite(n)){notify('数字格式不正确');return}
    if(valueToMs(n,unit)===null){notify('该时间值过大');setError('时间数值过大');return}
    currentParts.push({kind:'unit',unit,value:n});
  ...
  function pressColon(){
    ...
    if(!colonMode){
      ...
      if(numberBuffer.length>15){notify('小时位数过长');return}
      colonHours=String(Number(numberBuffer));numberBuffer='';
  ...
  function finishColon(){
    ...
    const p=colonPart();
    if(partMs(p)===null){notify('该时间值过大');setError('时间数值过大');return false}
    currentParts.push(p);
  ...
  function inputOperator(op){
    ...
    if(partsMs(currentParts)===null){notify('当前时间过大');return}
  ...
  function equals(){
    ...
    saveHistoryRecord(r.value);
    render();normalizedEl.innerHTML='';
  }
  ...
  function backspace(){
    ...
    if(colonMode){
      ...
      const last=rows.pop();
      currentOp=last.op;
      currentParts=clone(last.parts);
      if(!rows.length)currentOp=null;
    }
    render();
  }
  ...
  function toggleSelectedOp(){
    ...
    notify(`已改为 ${rows[selectedRow].op==='-'?'减':'加'}`);
  }
  function deleteSelectedRow(){
    if(selectedRow===null)return;
    rows.splice(selectedRow,1);
    if(rows.length)rows[0].op=null;
    selectedRow=null;partEdit=null;render();notify('已删除这一行');
  }
